...
using EPICSsharp.CA.Server.ChannelTypes ;
...
namespace EPICSsharp.CA.Server
{
  ...
  internal class ServerHandleMessage : DataFilter
  {
    ...
    public override void ProcessData ( DataPacket packet )
    {
      Console.WriteLine(
        "Pipe " 
      + ( Pipe.FirstFilter is UdpReceiver ? "UDP" : "TCP" ) 
      + ": " 
      + ( (CommandID) packet.Command )
      ) ;
      lock (lockObject)
      {
        ...
        case CommandID.CA_PROTO_READ_NOTIFY:
          {
            DataPacket response = DataPacketBuilder.Encode(
              ...
              ( 
                (ServerTcpReceiver) this.Pipe.FirstFilter
              ).RecordValue(
                this.Server, 
                packet.Parameter1
              ),
