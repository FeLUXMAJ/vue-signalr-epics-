        ...
        case CommandID.CA_PROTO_SEARCH:
          {
            ...
            if ( packet.Parameter1 != 0xFFFFFFFF )
              addr = IPAddress.Parse(
                "" + packet.Data[8] + "." + packet.Data[8 + 1] + "." + packet.Data[8 + 2] + "." + packet.Data[8 + 3]
              ) ;
            ...
            if ( Client.Configuration.DebugTiming )
            {
              lock ( channel.ElapsedTimings )
              {
                if ( ! channel.ElapsedTimings.ContainsKey("SearchResponse") )
                  channel.ElapsedTimings.Add(
                    "SearchResponse", 
                    channel.Stopwatch.Elapsed
                  ) ;
              }
            }
            if ( channel != null )
        ...
        case CommandID.CA_PROTO_ACCESS_RIGHTS:
          {
            ...
            if ( channel != null )
              channel.AccessRight = (AccessRights) (
                (ushort) packet.Parameter2
              ) ;
            break ;
          }
        ...
        case CommandID.CA_PROTO_SERVER_DISCONN:
          {
            ...
            ClientTcpReceiver receiver = (
              (ClientTcpReceiver) Pipe[0]
            ) ;
            lock ( receiver.ConnectedChannels )
            {
              connectedChannels = receiver.ConnectedChannels.Where(
                row => row.CID == packet.Parameter1
              ).ToList() ;
            }
