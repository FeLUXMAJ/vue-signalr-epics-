...
namespace EPICSsharp.CA.Common
{
  ...
  public class DataPacket : ICloneable, IDisposable
  {
        ...
        else
        {
          ...
          if ( value > 16000 )
          {
            ...
            if ( oldPacket.PayloadSize > 0 )
            {
              Buffer.BlockCopy(
                oldPacket.Data, 
                (int) oldPacket.HeaderSize, 
                Data, 
                24, 
                (int) oldPacket.PayloadSize
              ) ;
            }
    ...
    public bool HasCompleteHeader
    {
      get
      {
        if (
           Data.Length < 16 
        || ExtendedMessage && Data.Length < 24
        ) {
          return false ;
        }
        return true ;
    ...
    public string GetDataAsString ( int offset = 0, int maxSize = 40 )
    {
      ...
      string ret = Encoding.Default.GetString(
        Data, 
        offset, 
        Math.Min(
          Data.Length - offset, 
    ...
    // Returns a UInt32 at a given position
    ...
    // Creates a new message based on the byte buffer 
    ...
    // Creates a new message based on the byte buffer 
    // however use only the first "size" byte for it.
    ...
    // Creates a new message based on an existing packed 
    // and use the "size" to extract only the first part.
